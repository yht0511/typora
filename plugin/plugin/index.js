...
*/
  ...
  global._plugins = [
    ...
    },
    {
      name: "博客管理",
      fixed_name: "blog",
      src: "./plugin/tclab/tclab_blog.js",
      enable: false,
      clickable: false,
    },
  ];
  ...
  global._getPlugin = (fixed_name) => {
    const idx = global._plugins.findIndex(
      (plugin) => plugin.enable && plugin.fixed_name === fixed_name
    );
    if (idx !== -1) {
      ...
      const promise = new Promise((resolve, reject) => {
        _fs.access(filepath, (err) => {
          if (!err) {
            ...
            plugin.hide = hide || null;
            plugin.call_args = callArgs || null;
            plugin.dynamic_call_args_generator =
              dynamicCallArgsGenerator || null;
