(() => {
  ...
  const appendFirst = () => {
    ...
    const li = `
            <li data-key="typora-plugin" data-group="enable-plugin" class="has-extra-menu">
                <a role="menuitem">
                    <span data-localize="插件列表" data-lg="Menu">插件列表</span>
                    <i class="fa fa-caret-right"></i>
                </a>
            </li>
            <li data-key="typora-plugin-2" data-group="enable-plugin-2" id="Clear_All_Plugins">
                <a role="menuitem-2">
                    <span data-localize="清除插件" data-lg="Clear">清除插件</span>
                </a>
            </li>
            `;
  ...
  const appendSecond = (clickablePlugins, nonClickablePlugins) => {
    const clickable = clickablePlugins
      .map((plugin) => createSecondLi(plugin))
      .join("");
    const nonClickable = nonClickablePlugins
      .map((plugin) => createSecondLi(plugin))
      .join("");
  ...
  const appendThird = (enablePlugins) => {
    enablePlugins.forEach((plugin) => {
      ...
      thirdUl.innerHTML = plugin.call_args
        ? plugin.call_args.map((arg) => createThirdLi(arg)).join("")
        : "";
  ...
  const createSecondLi = (plugin) => {
    ...
    const style = plugin.clickable
      ? ""
      : `style="pointer-events: none;color: #c4c6cc;"`;
    const content = hasNotArgs
      ? plugin.name
      : `<span data-lg="Menu">${plugin.name}</span> <i class="fa fa-caret-right"></i>`;
  ...
  const generateDynamicCallArgs = (fixedName) => {
    ...
    if (plugin && plugin.enable && plugin.dynamic_call_args_generator) {
      const anchorNode = File.editor.getJQueryElem(
  ...
  const appendThirdLi = (menu, dynamicCallArgs) => {
    const args = dynamicCallArgs
      .map((arg) => createThirdLi(arg, true))
      .join("");
    menu.append(args);
  };
  ...
  const listen = (enablePlugins) => {
    opened_plugins = [];
    // 清除全部插件
    $("#Clear_All_Plugins").on("click", function () {
      opened_plugins.forEach((fixed_name) => {
        const plugin = enablePlugins.filter(
          (plugin) => plugin.fixed_name === fixed_name
        )[0];
    ...
    $("#plugin-menu")
      .on("click", "[data-key]", function () {
        ...
        if (opened_plugins.indexOf(fixed_name)==-1) opened_plugins.push(fixed_name);
        const plugin = enablePlugins.filter(
          (plugin) => plugin.fixed_name === fixed_name
        )[0];
      ...
      .on("mouseenter", "[data-key]", function () {
        const t = $(this);
        document
          .querySelectorAll(".plugin-menu-third")
          .forEach((ele) => ele.classList.remove("show"));
        document
          .querySelectorAll(".plugin-dynamic-arg")
        ...
        if (t.find(`span[data-lg="Menu"]`).length) {
          ...
          document
            .querySelector("#plugin-menu .plugin-has-args")
    ...
    $("#context-menu").on("mouseenter", "[data-key]", function () {
      ...
      if ("typora-plugin" === target.attr("data-key")) {
        ...
        document
          .querySelector("[data-key='typora-plugin']")
          .classList.remove("active");
        document
          .querySelectorAll(".plugin-menu-third")
          .forEach((ele) => ele.classList.remove("show"));
    ...
    $(".plugin-menu-third").on("click", "[data-key]", function () {
      ...
      const plugin = enablePlugins.filter(
        (plugin) => plugin.fixed_name === fixedName
      )[0];
