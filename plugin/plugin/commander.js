(() => {
  ...
  const config = {
    ...
    BUILTIN: [
      ...
      },
      {
        name: "BlogCreate",
        shell: SHELL.CMD_BASH,
        cmd: `blog create blog [\\"博客\\"] $f`,
      },
      {
        name: "BlogDelete",
        shell: SHELL.CMD_BASH,
        cmd: `blog delete <href>`,
      },
      {
        name: "BlogModify",
        shell: SHELL.CMD_BASH,
        cmd: `blog modify <href> blog [\\"博客\\"] $f`,
      },
    ],
  };
  ...
  (() => {
    ...
    const windowOption = File.isMac
      ...
      : `
            ...
            <option value="${SHELL.GIT_BASH}">Git</option>
            <option value="${SHELL.WSL}">WSL</option>
            <option value="${SHELL.SSH}">SSH</option>
        `;
    const builtin = config.BUILTIN.map(
      (ele) =>
        `<option shell="${ele.shell}" value='${ele.cmd}'>${ele.name}</option>`
    ).join("");
    const builtinSelect = !config.USE_BUILTIN
      ? ""
      : `<select class="typora-commander-builtin">${builtin}</select>`;
    ...
    const div = `
        <div id="typora-commander-form">
            ...
            <select class="typora-commander-shell"><option value="${SHELL.CMD_BASH}">CMD</option>${windowOption}</select>
            ${builtinSelect}
        </div>
    ...
    if (!config.USE_BUILTIN) {
      ...
      document.querySelector("#typora-commander-form input").style.width =
        "80%";
  ...
  const modal = {
    ...
    shellSelect: document.querySelector(
      "#typora-commander-form .typora-commander-shell"
    ),
    builtinSelect: document.querySelector(
      "#typora-commander-form .typora-commander-builtin"
    ),
  ...
  const getFolder = (shell) =>
    convertPath(Package.path.dirname(_getFile()), shell);
    ...
    Package.child_process.exec(
      ...
      (err, stdout, stderr) => {
        if (err || stderr.length) {
          ...
          if(stdout.startsWith('<BASE64>')){
            stdout= Base64.decode(stdout.substring(7, stdout.length));
          }
          resolve(stdout);
        }
      }
  ...
  const getFilePath = () =>
    File.filePath || (File.bundle && File.bundle.filePath);
  ...
  modal.modal.addEventListener("keydown", (ev) => {
    switch (ev.key) {
      ...
      case "Escape":
        ...
        break;
      case "Tab":
        const targetClass = config.USE_BUILTIN
          ? ".typora-commander-builtin"
          : ".typora-commander-shell";
  ...
  if (config.USE_BUILTIN) {
    modal.builtinSelect.addEventListener("change", (ev) => {
      const option =
        modal.builtinSelect.options[modal.builtinSelect.selectedIndex];
